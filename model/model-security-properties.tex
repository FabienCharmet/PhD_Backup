...
We postulate that the meaning of accuracy and completeness in our context is equivalent to considering that information and processes have only been altered by an authorized user or not altered at all\GB{this is quite similar to confidentiality with the exception that access is replaced with processing}.\FC{Indeed but integrity uses the notion of process in its definition where confidentiality doesn't}\GB{but in the above definition, the emphasis is on altering the process, while I felt in later definitions that the focus was on the alterations made or not by the process to the data. Which is it?}\FC{Later properties focus on network elements, but do not leave the process out of the definition}
...
\item reads(U,D,t)
\newline
This predicate is true if a monitoring tool intercepts traffic, or if the traffic is forwarded to a Virtual Machine, owned by either a legitimate or malicious user. Inspecting flow rules in nodes determines where the data was forwarded.
