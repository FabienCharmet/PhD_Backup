...
Several VNE algorithms are proposed, including one tailored to fit a type of topology that is expected to be common\GB{the expression is lengthy and confusing. please rewrite.}.
...
We\GB{``We''???} differentiate \textbf{slice} from \textbf{Virtual Networks (VN)} in~\cite{Zhao2017}.
...
Since the idea is to propose a ``Big Switch" abstraction while  the virtual resource is being replicated across several physical nodes\GB{The sentence starts with ``Since'', but no main clause is introduced.}. The point is to make sure that all nodes located on the edge of the Big Switch are preserving the ordering of the packets even when the configuration is updated.
