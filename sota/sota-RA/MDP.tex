...
The system is described by the set of state it can be in, and when in a particular state the agent will choose an action to perform\GB{too long and confusing: please split the sentence.}.
...
Such approach is considered in~\cite{Liang2012} where the authors use an MDP to handle resource transfer between mobile cloud domains. Each cloud domain composing the network can host a certain amount of services using its own resources\GB{sentence is too long and confusing: please split it.}. 
...
The resource allocation problem has been investigated using two\GB{but not limited to these two approaches...} different formalisms, namely the Game Theory formalism and Markov Decision Processes.
...
A strong advantage\GB{if there is already an obvious advantage of using MDP over GT in RA, was it worth investigating?} of MDPs over Game Theory models lies in the flexibility of the formalism to describe the available actions.
