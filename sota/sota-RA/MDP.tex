        ...
        \begin{algorithm}
            \Repeat{$\Delta < \Theta$ (a small positive number)}{
                ...
                \ForEach{$s\in S$}{
                    $temp = U(s)$\\
                    \tcc{Compute utility}
                    $V(s) = P(s,a,s')[R(s,a,s') + \gamma \sum_{s'}V(s')]$\\
...
\hline
Reference          & States                      & Actions                         & Summary                                         \\ \hline
\cite{Liang2011}   & Consumed resources          & Acceptation/Refusal of requests & Security services allocation in the cloud       \\ \hline
\cite{Liang2012}   & Number of services deployed & Accept/Reject/Transfer requests & Service allocation in mobile cloud networks     \\ \hline
\cite{Tesauro2006} & Available servers           & Allocation decision             & Offline training of an MDP using queuing models \\ \hline
\end{tabular}%
}
