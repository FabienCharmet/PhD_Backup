...
\textbf{Cooperative games} are games where all players will collaborate to maximize their personal gain, in opposition to non-cooperative games where players only maximize their own benefit\GB{I think you just want to say they do not collaborate. Because it seems that in cooperative and non-cooperative games, players always seek to maximize their own gain.}.
...
Game Theory has been applied to network security in different ways (e.g., IDS configuration and optimization), as well as, to monitoring resource allocation problems. We will focus on the latter. We summarized the different works in Table~\ref{tab:sota-gt}\GB{the table still does not include information on resolution}.
...
Koadialam~-~\cite{MuraliKodialam2003},~Otrok~-~\cite{otrok1,otrok2} & Static game        & Link sampling allocation for IDS                 \\ \hline
Agah~-~\cite{agah2004}                         & Static game        & Monitoring resource allocation in a sensor network \\ \hline
Chen~-~\cite{Chen2009,interdep-ismail2017}      & Static game        & Resource allocation  in an infrastructure          \\ \hline
Zhu~-~\cite{Zhu2009b}                         & Collaborative game & Trust management in untrusted environments         \\ \hline
Sallhammar~-~\cite{sallhammar2005}                  & Stochastic game    & \multirow{2}{*}{Attack modeling for resource allocation}             \\ \cline{1-2}
Nguyen~-~\cite{Nguyen2009}                       & Stochastic game    &                                                    \\ \hline
\end{tabular}%
}
