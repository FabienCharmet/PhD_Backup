...
\paragraph{Model assumptions}\textbf{\\}
\label{sec:mdp-model-assumption}

\textbf{Monitoring}\textbf{\\}
\begin{itemize}
    \item The deployment of the monitoring on the nodes impacts the infrastructure's performance. Based on the work of Ismail \etal~\cite{interdep-ismail2017}, we consider the monitoring cost  proportional to the intrinsic value of the nodes (\eg CPU time on a powerful machine is more expensive compared to a smaller one). 
...
The option of doing nothing prevents counter-productive options like forcing undesired actions (\eg unmonitoring a node that is not currently monitoring the infrastructure).
...
\paragraph{Cost of actions}
Choosing an action impact both $b_f$ and $b_c$ resources. The action will maje the migration progress by one step.\\
...
Based on the work of Ismail~\etal~\cite{interdep-ismail2017} we consider that the cost of monitoring each node is not uniform and depends on its intrinsic value. We represent this by assigning a proportionality coefficient to each node.
...
\begin{algorithm}
 ...
 \tcc{Is the node part of the embedding  and not already compromised ?}
  \uIf{$j \in $\textbf{W} and $Mi^s \cap At^s = \emptyset$}{
   $q(j) = \alpha  \frac{3}{3|\textbf{W}| + |\textbf{T}|}$
   }
  \tcc{Is the node part of the path to exfiltrate data ?}
  \uElseIf{$j \in $\textbf{T}} {
   $q(j) = \alpha  \frac{1}{3|\textbf{W}| + |\textbf{T}|}$
  }
  \Else{
  $q(j)=0$
  }
...
We note $q(j)$ the probability of node $j$ to be attacked as the combination of the probability for an attack to be launched (\ie $\alpha$) and the probability of node $j$ to be chosen among all the nodes in $\textbf{T}$ and $\textbf{W}$. We also use a coefficient (here 3) to give more weight to the nodes in $\textbf{W}$, referring to the assumption made in~\ref{sec:mdp-model-assumption}.
...
when choosing action $m_i$ at state $s$, we compute the resources impact and set changes for state $s', \forall j \in \textbf{N}$. One time step is deducted from $b_f$, the cost of monitoring node $i$ is deducted from $b_c$. Node $i$ is added to the set of monitoring nodes $Mo$ and finally, in case of an attack on node $j$, this node is added to the set of compromised nodes $At$.
...
when choosing action $d$ at state $s$, we compute the resources impact and set changes for state $s', \forall j \in \textbf{N}$. One time step is deducted from $b_f$ and in case of an attack on node $j$, this node is added to the set of compromised nodes $At$.
\begin{equation}
  s \longrightarrow s' =\begin{cases}
    b_f^{s'} = b_f^s - c_f\\
    At^{s'} = At^s \cup \{j\}\text{~~if there is an attack}
...
This differentiation comes from the fact that the attacker truly benefit from his attack once the full exfiltration path is established. Before that, the threat is focused on the next node to attack.
...
\\We note $\Pi(j)=1 - (1-p)^{|L_j \cap Mo|}$ as the probability of detecting the attack on node $j$ by at least one node .
\\
