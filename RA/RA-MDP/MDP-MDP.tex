...
The option of doing nothing prevents counter-productive options like forcing undesired actions (\eg removing monitoring from a node that is not currently monitoring the infrastructure) \CK{N'est-ce pas une action incoherente en l'occurence ?}\FC{justement, une action incoherente a une recompense nulle et donc il faut avoir la possibilite de ne rien faire qui est recompensee normalement}.
...
We define the transition probability with $\alpha$ \GB{is this $\alpha$ the one in Alg.~\ref{algo:target}?}\FC{Yes.} and $q(j)$ presented in Section~\ref{sec:target_proba}.
