...
Cloud Computing provides an interface for clients to operate their business, offers customization, and relieves them from the burden of the daily maintenance of the physical infrastructure. Recent advances in networking technologies have introduced 5G technologies, where Virtual Networks will be supported by multiple and heterogeneous technologies (\eg wireless and optical networks)\GB{no! 5G is not supported by wireless and optical networks. you may refer as a radio and optical infra, but it does not support 5G per se.}.
...
\GB{what is the logical link between the two paragraphs?}
...
\begin{itemize}
    \item What is the security of the VN migration process?\GB{bad research question. I suggest ``How can we express/evaluate the security of the migration process?}
...
\GB{you need to state how the two goals are related and please highlight this part (using bold font for example).}
